...
class ESConnection(ESConnectionBase):
    ...
    def map_message_to_es_fields(message: dict) -> dict:
        ...
        from rag.nlp.rag_tokenizer import tokenize, fine_grained_tokenize
        storage_doc["tokenized_content_ltks"] = fine_grained_tokenize(tokenize(message["content"]))
        storage_doc[f"q_{len(message['content_embed'])}_vec"] = message["content_embed"]
        return storage_doc
